...
- Physical address: not yet provided by the client — use the literal placeholder
  text `[Add your office address here]` anywhere an address would appear. Never
  invent a fake real-sounding street address.
...
- Ink (permanently-dark surfaces — navbar, hero, footer, dark bands): `#0D1B26` —
  `var(--ink)` / `var(--ink-2)` / `var(--ink-border)`. These do NOT flip with the
  light/dark theme toggle — the header, hero, and footer stay dark ink regardless
  of site theme, by design. Text/icons on those bands use `var(--on-ink)` /
...
- Logo mark: `./favicon.svg` — a flat ink hexagon "cargo seal" badge (`#0D1B26`)
  ...
  js/components/header.js / footer.js) — the `<span>` now renders in `--accent`
  amber instead of `--primary`, since the navbar/footer are permanently dark ink.
  Keep the mark flat — no gradient fills, drop shadows, or glow.
